...
class SecretHandshake
  attr_accessor :number, :number_arr, :normalized_num
  COMMANDS = ["wink", "double blink", "close your eyes", "jump"]

  def initialize(number)
    @number = number
    @number_arr = number.chars
    normalize(number)
  end

  def normalize(number)
   (5-number.length).times {number_arr.unshift("0")}
   self.number_arr.reverse!
  end

  def commands
    gestures = []
    self.number_arr.each_with_index do |num, i|
      unless i == 4
        gestures << COMMANDS[i] if num == "1"
      else
        gestures.reverse! if num == "1"
      end
    end
    gestures
  end
